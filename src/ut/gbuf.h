...
static inline int ut_gbuf_ialloc(
	struct ut_gbuf *gbuf
) {
	if (!gbuf)
		goto fail_struct;
...
static inline int ut_gbuf_init(
	struct ut_gbuf *gbuf
) {
	if (!gbuf)
		goto fail_struct;
...
static inline void ut_gbuf_sfree(
	struct ut_gbuf *gbuf
) {
	CFG_FREE(gbuf);
	return;
...
static inline void ut_gbuf_ifree(
	struct ut_gbuf *gbuf
) {
	CFG_FREE(gbuf->ptr);
	return;
...
static inline void ut_gbuf_ffree(
	struct ut_gbuf *gbuf
) {
	ut_gbuf_ifree(gbuf);
	ut_gbuf_sfree(gbuf);
...
static inline int ut_gbuf_grow(
	struct ut_gbuf *gbuf
) {
...
static inline int ut_gbuf_push(
	struct ut_gbuf *gbuf,
	void *val
) {
	// grow if needed
