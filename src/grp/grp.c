...
int grp_cpu_groups_push(void) {
  ...
  for (int i = 0; i < sockets; i++) {
    ...
    const char *match[] = {model, NULL};

    snprintf(id, sizeof(id), "%d", i);

    if (hw_dev_init( // nothing matched
            dev, id, match, SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY)) {
      hw_dev_init( // so use a generic
          dev, id, (const char *[]){"generic_cpu", NULL},
          SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
    }
...
int grp_motherboard_groups_push(void) {
  ...
  const char *match[] = {string, NULL};

  if (hw_dev_init( // nothing found
          dev, "0", match, SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY)) {
    hw_dev_init( // so use a generic
        dev, "0", (const char *[]){"generic_motherboard", NULL},
        SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
  }
...
int grp_dram_groups_push(void) {
  ...
  struct udev_device *udev_dev =
      udev_device_new_from_syspath(udev, "/sys/devices/virtual/dmi/id");
  ...
  while (1) {
    ...
    const char *model = udev_device_get_property_value(udev_dev, key);

    snprintf(key, sizeof(key), "MEMORY_DEVICE_%d_MANUFACTURER", i);
    const char *vendor = udev_device_get_property_value(udev_dev, key);
    i++;
    ...
    if (hw_dev_init( // nothing found
            dev, "0", match, SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY)) {
      hw_dev_init( // so use a generic
          dev, "0", (const char *[]){"generic_dram", NULL},
          SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
    }
...
int grp_storage_groups_push(void) {
  ...
  udev_list_entry_foreach(udev_entry, udev_devices) {
    ...
    if (!udev_name || strncmp(udev_name, "/dev/loop", 9) == 0 ||
    ...
    struct udev_device *usb =
        udev_device_get_parent_with_subsystem_devtype(udev_dev, "usb", NULL);
    if (usb) {
      grp_group_destroy(storage);
      CFG_FREE(dev);
      udev_device_unref(udev_dev);
    ...
    CFG_STRNCPY(string, "", sizeof(string));
    const char *vendor =
        udev_device_get_sysattr_value(udev_dev, "device/vendor");
    if (vendor)
      CFG_STRNCAT(string, vendor, sizeof(string));
    ...
    if (hw_dev_init( // nothing found
            dev, udev_path, match, SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY)) {
      hw_dev_init( // so use a generic
          dev, udev_path, (const char *[]){"generic_storage", NULL},
          SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
    }
...
int grp_gpu_groups_push(void) {
  ...
  udev_list_entry_foreach(udev_entry, udev_devices) {
    ...
    const char *board_vendor =
        udev_device_get_sysattr_value(udev_dev, "subsystem_vendor");
    const char *silicon_vendor =
        udev_device_get_sysattr_value(udev_dev, "vendor");
    const char *model =
        udev_device_get_property_value(udev_dev, "ID_MODEL_FROM_DATABASE");
    snprintf(id, sizeof(id), "%d", 0);

    const char *match[] = {model, silicon_vendor, board_vendor, NULL};

    hw_dev_init(dev, id, match, SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
...
int grp_nic_groups_push(void) {
  ...
  udev_list_entry_foreach(udev_entry, udev_devices) {
    ...
    hw_dev_init(dev, udev_path, match, SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
...
int grp_fan_groups_push(void) {
  ...
  while ((udev_path = strsep(&hwmons, " ")) != NULL) {
    ...
    udev_list_entry_foreach(udev_entry, udev_devices) {
      ...
      const char *match[] = {"generic_fan", NULL};
