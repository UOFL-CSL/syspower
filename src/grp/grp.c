...
int grp_cpu_groups_push(void)
{
	...
	for (int i = 0; i < sockets; i++) {
		...
		obj = hwloc_get_obj_by_type(topology, HWLOC_OBJ_PACKAGE,
					    (unsigned int)i);
		...
		if (hw_dev_init( // nothing matched
			    dev, id, match, SYSPWR_HW_LOCALE_LOCAL,
			    CFG_EFFICIENCY)) {
			hw_dev_init( // so use a generic
				dev, id,
				(const char *[]){ "generic_cpu", NULL },
...
int grp_dram_groups_push(void)
{
	...
	struct udev_device *udev_dev = udev_device_new_from_syspath(
		udev, "/sys/devices/virtual/dmi/id");
	...
	while (1) {
		...
		const char *model =
			udev_device_get_property_value(udev_dev, key);
		...
		if (hw_dev_init( // nothing found
			    dev, "0", match, SYSPWR_HW_LOCALE_LOCAL,
			    CFG_EFFICIENCY)) {
			hw_dev_init( // so use a generic
				dev, "0",
				(const char *[]){ "generic_dram", NULL },
				SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
		}
...
int grp_storage_groups_push(void)
{
	...
	udev_list_entry_foreach(udev_entry, udev_devices)
	{
		...
		const char *udev_type =
			udev_device_get_property_value(udev_dev, "DEVTYPE");
		...
		const char *udev_name =
			udev_device_get_property_value(udev_dev, "DEVNAME");
		...
		const char *vendor = udev_device_get_sysattr_value(
			udev_dev, "device/vendor");
		if (vendor)
			CFG_STRNCAT(string, vendor, sizeof(string));
		const char *model =
			udev_device_get_sysattr_value(udev_dev, "device/model");
		if (vendor && model)
			CFG_STRNCAT(string, " ", sizeof(string));
		if (model)
			CFG_STRNCAT(string, model, sizeof(string));
		...
		if (hw_dev_init( // nothing found
			    dev, udev_path, match, SYSPWR_HW_LOCALE_LOCAL,
			    CFG_EFFICIENCY)) {
			hw_dev_init( // so use a generic
				dev, udev_path,
				(const char *[]){ "generic_storage", NULL },
				SYSPWR_HW_LOCALE_LOCAL, CFG_EFFICIENCY);
		}
...
int grp_gpu_groups_push(void)
{
	...
	udev_list_entry_foreach(udev_entry, udev_devices)
	{
		...
		const char *udev_class =
			udev_device_get_sysattr_value(udev_dev, "class");
		if (!udev_class ||
		    !( // not a GPU
			    strncmp(udev_class, "0x03", 4) == 0 // general
			    || strncmp(udev_class, "0x12", 4) ==
				       0 // AI accelerator
			    )) {
			grp_group_destroy(gpu);
			CFG_FREE(dev);
		...
		const char *board_vendor = udev_device_get_sysattr_value(
			udev_dev, "subsystem_vendor");
		const char *silicon_vendor =
			udev_device_get_sysattr_value(udev_dev, "vendor");
		const char *model = udev_device_get_property_value(
			udev_dev, "ID_MODEL_FROM_DATABASE");
		snprintf(id, sizeof(id), "%d", 0);

		const char *match[] = { model, silicon_vendor, board_vendor,
					NULL };

		hw_dev_init(dev, id, match, SYSPWR_HW_LOCALE_LOCAL,
			    CFG_EFFICIENCY);
...
int grp_nic_groups_push(void)
{
	...
	udev_list_entry_foreach(udev_entry, udev_devices)
	{
		...
		hw_dev_init(dev, udev_path, match, SYSPWR_HW_LOCALE_LOCAL,
			    CFG_EFFICIENCY);
...
int grp_fan_groups_push(void)
{
	...
	while ((udev_path = strsep(&hwmons, " ")) != NULL) {
		...
		udev_list_entry_foreach(udev_entry, udev_devices)
		{
			...
			const char *pwm_string =
				udev_device_get_sysattr_value(udev_dev, name);
			if (!pwm_string)
				continue;
			long long int pwm;
