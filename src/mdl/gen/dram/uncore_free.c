...
{
	return syscall(SYS_perf_event_open, hw_event, pid, cpu, group_fd,
		       flags);
}
...
static int64_t bytes_per_counter(char *path)
{
	...
	} else if (strncmp(buffer, "Bytes", 5) == 0 ||
		   strncmp(buffer, "B", 1) == 0) {
		scale *= 1.0;
	} else {
		goto fallback;
	}
...
static int configure(struct grp_group *grp)
{
	...
	for (uint32_t i = 0; i < grp->hw.len; i++) {
		...
		const char *read_events[] = { "data_read", "data_reads",
					      "cas_count_read" };
		char rpath[256];
		int found_r = 0;
		for (size_t e = 0;
		     e < sizeof(read_events) / sizeof(read_events[0]); e++) {
			snprintf(rpath, sizeof(rpath), "%s/events/%s", base,
				 read_events[e]);
		...
		const char *write_events[] = { "data_write", "data_writes",
					       "cas_count_write" };
		char wpath[256];
		int found_w = 0;
		for (size_t e = 0;
		     e < sizeof(write_events) / sizeof(write_events[0]); e++) {
			snprintf(wpath, sizeof(wpath), "%s/events/%s", base,
				 write_events[e]);
...
int mdl_gen_dram_intel_uncore_free(struct grp_group *grp)
{
	...
	for (uint32_t i = (uint32_t)grp->counter;
		...
		if (bw > entry->bw_max)
			entry->bw_max = bw;
		grp->jw.uw +=
			props.uw_min + (props.uw_max * bw) / entry->bw_max;
	}
