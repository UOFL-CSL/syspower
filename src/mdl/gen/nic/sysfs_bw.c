	...
	for (uint32_t i = 0; i < grp->hw.len; i++) {
		...
		while ((dentry = readdir(dir)) != NULL) {
			if (!(dentry->d_name[0] == '.' 
				...
				CFG_STRNCPY(path, dev->id, sizeof(path) - 1);
				CFG_STRNCAT(path, "/net/", sizeof(path) - 1);
				CFG_STRNCAT(path, dentry->d_name, sizeof(path) - 1);
				CFG_STRNCAT(path, "/speed", sizeof(path) - 1);
				fd = open(path, O_RDONLY);
				if (fd < 0) goto fail_dir;
				int64_t bw;
				ssize_t bytes = read(fd, buffer, sizeof(buffer) - 1);
				close(fd);
				if (bytes < 0) goto fail_dir;
				if (sscanf(buffer, "%ld", &bw) != 1)
					goto fail_dir;
				bw *= (1000 * 1000) / 8;
				lentry = CFG_CALLOC(1, sizeof(struct link));
				if (!lentry) goto fail_dir;
				...
				CFG_STRNCAT(path, "/net/", sizeof(path) - 1);
				CFG_STRNCAT(path, dentry->d_name, sizeof(path) - 1);
				CFG_STRNCAT(path, "/statistics/tx_bytes", sizeof(path) - 1);
				fd = open(path, O_RDONLY);
				if (fd < 0) goto fail_lentry;
				lentry->fdw = fd;
				CFG_STRNCPY(path, dev->id, sizeof(path) - 1);
				CFG_STRNCAT(path, "/net/", sizeof(path) - 1);
				CFG_STRNCAT(path, dentry->d_name, sizeof(path) - 1);
				CFG_STRNCAT(path, "/statistics/rx_bytes", sizeof(path) - 1);
				fd = open(path, O_RDONLY);
				if (fd < 0) goto fail_fdw;
				lentry->fdr = fd;
...
int mdl_gen_nic_sysfs_bw(
	...
	for (uint32_t i = (uint32_t)grp->counter;
		...
		for (uint32_t j = 0; j < nentry->links.len; j++) {
			...
			ssize_t bytes = read(lentry->fdr, buffer, sizeof(buffer) - 1);
			...
			if (!ptime)
				continue;
			int64_t bw = ((total - lentry->prev) * 1000 * 1000 * 1000)
				/ (grp->jw.uj_ts_ns - ptime);
			lentry->prev = total;
			if (bw > lentry->bw_max)
				lentry->bw_max = bw;
			grp->jw.uw += props.uw_min / (int64_t)nentry->links.len
				+ (props.uw_max * bw) / (int64_t)nentry->links.len
				/ lentry->bw_max;
		}
	}
